...
This package implements the self-updating process clustering algorithms proposed in @Shui2016supc and shows how to reproduce the examples and figures in the paper.
...
If `t` is a character value, only the first element will be used. This element should be either `"static"` or `"dynamic"`. If `t = "static"`, `supc1` will set `t` as `r quoted.string(show.obj(supc:::.t.static))` where `r` is the corresponding value of $r$ in $\eqref{eq:influence}$. This is a constant during iteration. Similarly, if `t = "dynamic"`, `supc1` will set `t` as `r quoted.string(show.obj(supc:::.t.dynamic))`. Mathematically, it is $T(t) = \frac{r}{20} + t \times \frac{r}{50}$. It will increase during the iteration.
...
Here we reproduce the section 2.3 of @Shui2016supc as an usage example.

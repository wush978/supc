...
if (!file.exists("dist.gif")) {
  ...
  local({
    ...
    animation::saveGIF(lapply(1:3, function(j) {
      plot(.x, rep(1, length(.x)), type = 'n', ylim = c(.y.min, .y.max), xlab = "number of variables", ylab = "computation time (seconds)", xaxt = 'n', yaxt = 'n')
      ...
      title(sub = sprintf("number of samples: %d", floor(10^seq(2, 3, by = 0.5)[j])))

...
The SUP is a distance-based method for clustering. The idea of this algorithm is similar to gravitational attraction: every samples gravitates toward one another. 
...
The iteration stops after convergence. We compare the $L_{\infty}$ distance (i.e. $max_{1 \leq i \leq N} {\left\lVert x_i^{(t)} - x_i^{(t + 1)} \right\rVert_{\infty}}$) with `tolerance`. If the $L_{\infty}$ distance is lower than `tolerance`, then the computation stops and returns $x_1^{(t)}, ..., x_N^{(t)}$.
...
The user can use these two vectors to identify the exact locations of the valleys of the polygon, which are at 0.9, 1.7, 2.1, 2.5, etc. in this data set.

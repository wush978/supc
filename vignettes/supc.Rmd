...
This vector represents the IDs of the resulting clusters of each instance. For example, the first instance, i.e. `X[1,]`, is clustered into cluster ID `X.supc$cluster[1]`.
...
The following graphs show clustering results using different `r` values. The IDs of the clusters each instance is clustered into is plotted in the graph. The major clusters are marked by blue circles. Note that the size of the major clusters increases with `r` value.

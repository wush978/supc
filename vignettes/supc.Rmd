...
Clustering analysis is a common technique to discover patterns and has been widely applied to many disciplines for partition data into groups. In the literature, different families of clustering algorithms have been developed. The model-based methods @Banfield1993 make an assumption on the probabilistic distribution of data, and the distance-based method relies on the similarity between samples. There are two major types of the distance-based methods. The first type is hierarchical clustering, which clusters data into groups through a series of operations that operate on the similarity measure between samples. The second type defines an objective criterion and clusters the data which optimizes the criterion. k-means is one of the most popular clustering method of the second type. 

The SUP is a distance-based method for clustering. The idea is similar to the gravity: every samples are graviate toward one another. The algorithm simulate the process of gravity iteratively and makes the samples merge into clusters on the sample space. During the iteration, all data points continue updating their positions until the system becomes stable. Therefore, the algorithm is named as Slef-Updating Process(SUP). More specifically, the user can tune the algorithm by changing the rule of the gravity.
...
The user can use `dist.mode` to select the package to compute the pairwise distance between samples. Specifying `dist.mode("stats")`, `dist.mode("amap")`, and `dist.mode("gputools")` will select `stats::dist`, `amap::Dist`, and `gputools::gpuDist` to compute the pairwise distance matrix, respectively. It is recommended to compare the computation time of `stats::dist(x)`, `amap::Dist(x)` and `gputools::gpuDist(x)` before selecting the function.
...
This package provide both implementation for better maintenance. The `C++` program runs faster when the number of samples is larger. 
...
After specifying the argument `breaks`, the user can control the number of bins. Please see `?hist` for details.

```{r}
X.freq <- freq.poly(X, breaks = 50)
```

More specifically, if the user wants to find the valley, please check the returned object from `freq.poly`. It is the same as the object returned by the `hist` function. In this example, the `X.freq$mids` is the vector of x-axis and `X.freq$count` is the vector of y-axis of the frequency plot.
...
- `cluster`, the cluster label of each samples.
...
Note that this clustering result shows that SUP identified 15 clusters. The three major clusters are consisted of data points from nine normally distributed groups. The rest twelve clusters are identified to be consisted of noise data points.
...
For demonstration, the following uses `r quoted.string("r = c(0.9, 1.7, 2.5)")` according to the approximate locations of sharp values in the frequency polygon of pairwise distance.
...
The results show that the use of `r quoted.string("r = 0.9")` produces 23 clusters, including nine major clusters and 14 tiny clusters that can be identified as noise. The use of `r quoted.string("r = 1.7")` produces 15 clusters, including three major clusters and 12 tiny clusters that can be identified as noise. When the influential range `r` is set as large as 2.5, the major clusters consist of mixed data from both the normally distributed data points and the noise data points.
...
```{r}
plot(X.supcs[[1]], type = "heatmap")
```

After specifying `type = "heatmap"`, the `plot` function will draw the corresponding heatmap of the given `supc` object.
The samples will be ordered by the cluster IDs. The dashed lines split the samples into different clusters and the cluster size is annotated above the figure.

The user could pass additional parameters to the underlying `image` function. For example:

```{r}
plot(X.supcs[[2]], type = "heatmap", col = cm.colors(24))
```
